"""Headless tests: LabelSet (colorize, IO) and GridTarget (apply/undo/clear)."""
...
def test_labelset_colorize_ignore_transparent():
    ...
    assert tuple(rgba[0, 1, :3]) == (60, 200, 70)  # class 1
    assert rgba[1, 0, 3] == 255  # class 2 opaque
...
def test_gridtarget_apply_and_undo():
    ...
    assert r[0, 0] == 0  # outside the rectangle
    t.undo(0)
...
def test_gridtarget_apply_mask_and_undo():
    ...
    mask[0, 0] = mask[3, 3] = mask[1, 2] = True  # non-contiguous cells (selection)
